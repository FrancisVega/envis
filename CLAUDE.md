...
- `npm start` — ejecuta el CLI ya construido (`node dist/cli.js`) sobre el directorio actual.
...
- **`src/cli.ts`** — entrada `npx`: arranca Hono (sirve API + el `index.html` único) y abre el navegador.
...
- `ENVIS_CONFIG_DIR` — dónde guardar el registro de proyectos (aislamiento en tests).
